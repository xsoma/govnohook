...
bool ImGui::CustomButton(const char* label, const char* label_id, const ImVec2& size_arg)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    RenderFrame(bb.Min, bb.Max, hovered ? ImColor(27, 27, 27, 255) : ImColor(32, 32, 32, 255), true);
    ImGui::GetWindowDrawList()->AddRect(bb.Min, bb.Max, ImColor(60, 60, 60, 255));
    ...
    //if (operation)
    //{
    //    if (pressed1) { *operation = OPERATION_RENAME; return true; }  //else operation = 0;
    //    if (pressed2) { *operation = OPERATION_REMOVE; return true; }  //else operation = 0;
    //    if (pressed3) { *operation = OPERATION_EDIT; return true; }  //else operation = 0;
    //    if (pressed4) { *operation = OPERATION_SAVE; return true; }  //else operation = 0;
    //    if (pressed5) { *operation = OPERATION_LOAD; return true; }  //else operation = 0;
    //    if (pressed6) { *operation = OPERATION_LOAD; return true; }  //else operation = 0;
    //    if (pressed7) { *operation = OPERATION_LOAD; return true; }  //else operation = 0;
    //    if (pressed8) { *operation = OPERATION_LOAD; return true; }  //else operation = 0;
    //    if (pressed9) { *operation = OPERATION_LOAD; return true; }  //else operation = 0;
    //}
    //else
    //{
    if (pressed) { /*operation = OPERATION_LOAD;*/ return true; }  //else operation = 0;
    //if (pressed2) { /*operation = OPERATION_REMOVE;*/ return true; }  //else operation = 0;
    //if (pressed3) { /*operation = OPERATION_EDIT;*/ return true; }  //else operation = 0;
    //if (pressed4) { /*operation = OPERATION_SAVE;*/ return true; }  //else operation = 0;
    //if (pressed5) { /*operation = OPERATION_LOAD;*/ return true; }  //else operation = 0;
...
{
    ImGuiContext& g = *GImGui;
    if (g.NextWindowData.SizeConstraintRect.GetWidth() != 0 && g.NextWindowData.SizeConstraintRect.GetHeight() != 0)
    {
...
void ImGui::TabButton(const char* label, int* selected, int num, int total) {
    ...
    ImVec2 pos = window->Pos + ImVec2(10, 30) + ImVec2(0, 30 * num);
    ...
    float deltatime = 1.0f * ImGui::GetIO().DeltaTime;

    const ImVec4 hovered_circle_active = ImVec4(ctx.m_settings.menu_color[0], ctx.m_settings.menu_color[1], ctx.m_settings.menu_color[2], 1.f);
    const ImVec4 hovered_circle_disable = ImVec4(ctx.m_settings.menu_color[0], ctx.m_settings.menu_color[1], ctx.m_settings.menu_color[2], 0.f);

    const ImVec4 hovered_background_active = ImVec4(42 / 255.f, 42 / 255.f, 42 / 255.f, 1.f);
    ...
    window->DrawList->AddCircleFilled(ImVec2(pos.x + size.x - 22, pos.y + size.y / 2), 5.f, ImGui::GetColorU32(it_hcolor->second));
    window->DrawList->AddText(ImVec2(pos.x, pos.y + size.y / 2 - label_size.y / 2 - 1), ImColor(200, 200, 200, 255), label);
}
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(pos + ImVec2(2, 0), pos + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, label_size.y));
    ItemSize(total_bb, style.FramePadding.y);
    ...
    const ImVec4 hover_act = ImVec4(ctx.m_settings.menu_color[0], ctx.m_settings.menu_color[1], ctx.m_settings.menu_color[2], 0.f);
    const ImVec4 hover_dis = ImVec4(ctx.m_settings.menu_color[0], ctx.m_settings.menu_color[1], ctx.m_settings.menu_color[2], 1.f);
    ...
    window->DrawList->AddRectFilled(ImVec2(pos), pos + ImVec2(12, 12), ImColor(26, 26, 30), 2);
    window->DrawList->AddRect(pos, pos + ImVec2(12, 12), ImColor(40 / 255.f, 40 / 255.f, 44 / 255.f, g.Style.Alpha), 2);
    window->DrawList->AddRect(pos - ImVec2(1, 1), pos + ImVec2(13, 13), ImColor(10 / 255.f, 10 / 255.f, 14 / 255.f, g.Style.Alpha), 2);

    window->DrawList->AddRectFilled(ImVec2(pos.x + 1, pos.y + 1), ImVec2(pos.x + 11, pos.y + 11), ImGui::GetColorU32(it_hcolor->second), 2);

    window->DrawList->AddText(pos + ImVec2(15, -2), ImGui::GetColorU32(ImGuiCol_Text), label);
...
bool ImGui::Keybind(const char* str_id, int* current_key, int* key_style) {
    ...
    const ImRect total_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(label_size.x, label_size.y));
    ItemSize(total_bb, style.FramePadding.y);
    ...
    if (g.ActiveId == id) {
        ...
        if (key_style) {
            ...
            if (popup_open) {
                ...
                if (Selectable("on hotley", *key_style == 1))
                    *key_style = 1;

                if (Selectable("force off", *key_style == 2))
                    *key_style = 2;

                if (Selectable("always", *key_style == 3))
                    *key_style = 3;
    ...
    window->DrawList->AddText(frame_bb.Min, g.ActiveId == id ? ImColor(ctx.m_settings.menu_color[0], ctx.m_settings.menu_color[1], ctx.m_settings.menu_color[2], ctx.m_settings.menu_color[3]) : ImColor(90 / 255.f, 90 / 255.f, 90 / 255.f, g.Style.Alpha), buf_display);
    PopFont();
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, int items)
{
    ...
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(0, 0), window->DC.CursorPos + ImVec2(w, label_size.y + 20));
    ...
    const ImRect value_bb(frame_bb.Min + ImVec2(140, 0), frame_bb.Max - ImVec2(arrow_size, 0.0f));
    ImRect cb_bb = ImRect(frame_bb.Min + ImVec2(0, label_size.y + 3), frame_bb.Max);
    ...
    window->DrawList->AddRectFilled(cb_bb.Min, cb_bb.Max, ImColor(25, 25, 29), 3);

    window->DrawList->AddRect(cb_bb.Min - ImVec2(1, 1), cb_bb.Max + ImVec2(1, 1), ImColor(10 / 255.f, 10 / 255.f, 14 / 255.f, g.Style.Alpha), 3);
    window->DrawList->AddRect(cb_bb.Min, cb_bb.Max, ImColor(40 / 255.f, 40 / 255.f, 44 / 255.f, g.Style.Alpha), 3);
    window->DrawList->AddRect(cb_bb.Min, cb_bb.Max, ImColor(ctx.m_settings.menu_color[0], ctx.m_settings.menu_color[1], ctx.m_settings.menu_color[2], it_hcolor->second.w), 3);

    if (label_size.x > 0)
        window->DrawList->AddText(frame_bb.Min + ImVec2(0, -1), ImColor(205 / 255.f, 205 / 255.f, 205 / 255.f, g.Style.Alpha), label);

    window->DrawList->PushClipRect(cb_bb.Min, cb_bb.Max - ImVec2(20, 0), true);
    window->DrawList->AddText(cb_bb.Min + ImVec2(5, -1), ImColor(205 / 255.f, 205 / 255.f, 205 / 255.f, g.Style.Alpha), preview_value);
    ...
    SetNextWindowSize(ImVec2(w, CalcMaxPopupHeightFromItemCount(items)));
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(0, 0), window->DC.CursorPos + ImVec2(w, label_size.y + 1));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max);
    ...
    ImRect slider_bb = ImRect(frame_bb.Min + ImVec2(160, 6), frame_bb.Max - ImVec2(0, 6));
    ...
    window->DrawList->AddRectFilled(ImVec2(slider_bb.Min.x, slider_bb.Min.y), ImVec2(slider_bb.Max.x, slider_bb.Max.y), ImColor(26, 26, 30), 2);
    window->DrawList->AddRect(ImVec2(slider_bb.Min.x, slider_bb.Min.y), ImVec2(slider_bb.Max.x, slider_bb.Max.y), ImColor(40, 40, 44), 2);
    window->DrawList->AddRect(ImVec2(slider_bb.Min.x - 1, slider_bb.Min.y - 1), ImVec2(slider_bb.Max.x + 1, slider_bb.Max.y + 1), ImColor(10, 10, 14), 2);

    window->DrawList->AddRectFilled(slider_bb.Min, ImVec2(slider_bb.Min.x + floorf((float)percent / 100.f * (slider_bb.Max.x - slider_bb.Min.x)), slider_bb.Max.y), ImColor(ctx.m_settings.menu_color[0], ctx.m_settings.menu_color[1], ctx.m_settings.menu_color[2], ctx.m_settings.menu_color[3]), 2);
    //window->DrawList->AddCircleFilled(ImVec2(slider_bb.Min.x + floorf((float)percent / 100.f * (slider_bb.Max.x - slider_bb.Min.x)), slider_bb.Max.y - 3), 5.f, ImColor(ctx.m_settings.menu_color[0], ctx.m_settings.menu_color[1], ctx.m_settings.menu_color[2], ctx.m_settings.menu_color[3]));

    window->DrawList->AddRectFilled(ImVec2(slider_bb.Min.x + floorf((float)percent / 100.f * (slider_bb.Max.x - slider_bb.Min.x)) - 7, slider_bb.Min.y - 3), ImVec2(slider_bb.Min.x + floorf((float)percent / 100.f * (slider_bb.Max.x - slider_bb.Min.x)) + 7, slider_bb.Max.y + 3), ImColor(26, 26, 30), 2);
    window->DrawList->AddRect(ImVec2(slider_bb.Min.x + floorf((float)percent / 100.f * (slider_bb.Max.x - slider_bb.Min.x)) - 7, slider_bb.Min.y - 3), ImVec2(slider_bb.Min.x + floorf((float)percent / 100.f * (slider_bb.Max.x - slider_bb.Min.x)) + 7, slider_bb.Max.y + 3), ImColor(40, 40, 44), 2);
    window->DrawList->AddRect(ImVec2(slider_bb.Min.x + floorf((float)percent / 100.f * (slider_bb.Max.x - slider_bb.Min.x)) - 8, slider_bb.Min.y - 4), ImVec2(slider_bb.Min.x + floorf((float)percent / 100.f * (slider_bb.Max.x - slider_bb.Min.x)) + 8, slider_bb.Max.y + 4), ImColor(10, 10, 14), 2);

    window->DrawList->AddLine(ImVec2(slider_bb.Min.x + floorf((float)percent / 100.f * (slider_bb.Max.x - slider_bb.Min.x)) - 3, slider_bb.Min.y), ImVec2(slider_bb.Min.x + floorf((float)percent / 100.f * (slider_bb.Max.x - slider_bb.Min.x)) - 3, slider_bb.Max.y), ImColor(60, 60, 64));
    window->DrawList->AddLine(ImVec2(slider_bb.Min.x + floorf((float)percent / 100.f * (slider_bb.Max.x - slider_bb.Min.x)) - 0, slider_bb.Min.y), ImVec2(slider_bb.Min.x + floorf((float)percent / 100.f * (slider_bb.Max.x - slider_bb.Min.x)) - 0, slider_bb.Max.y), ImColor(60, 60, 64));
    window->DrawList->AddLine(ImVec2(slider_bb.Min.x + floorf((float)percent / 100.f * (slider_bb.Max.x - slider_bb.Min.x)) + 3, slider_bb.Min.y), ImVec2(slider_bb.Min.x + floorf((float)percent / 100.f * (slider_bb.Max.x - slider_bb.Min.x)) + 3, slider_bb.Max.y), ImColor(60, 60, 64));
    ...
    */
    ...
    window->DrawList->AddRectFilled(ImVec2(slider_bb.Min.x + floorf((float)percent / 100.f * (slider_bb.Max.x - slider_bb.Min.x)) - ImGui::CalcTextSize(label).x - 3, slider_bb.Min.y - 20), ImVec2(slider_bb.Min.x + floorf((float)percent / 100.f * (slider_bb.Max.x - slider_bb.Min.x) + ImGui::CalcTextSize(label).x + 3), slider_bb.Min.y - 3), ImColor(25, 25, 29, int(it_hcolor->second.w * 255)), 7);
    window->DrawList->AddRect(ImVec2(slider_bb.Min.x + floorf((float)percent / 100.f * (slider_bb.Max.x - slider_bb.Min.x)) - 10, slider_bb.Min.y - 20), ImVec2(slider_bb.Min.x + floorf((float)percent / 100.f * (slider_bb.Max.x - slider_bb.Min.x) + 10), slider_bb.Min.y - 3), ImColor(45, 45, 45, int(it_hcolor->second.w * 255)), 7);
    window->DrawList->AddText(ImVec2(slider_bb.Min.x + floorf((float)percent / 100.f * (slider_bb.Max.x - slider_bb.Min.x)) - textSize.x / 2, slider_bb.Min.y - 20), ImColor(205 / 255.f, 205 / 255.f, 205 / 255.f, it_hcolor->second.w), value_buf);
...
bool ImGui::VSliderScalar(const char* label, const ImVec2& size, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    // Slider behavior
    ImRect grab_bb;
    const bool value_changed = SliderBehavior(frame_bb, id, data_type, p_data, p_min, p_max, format, flags | ImGuiSliderFlags_Vertical, &grab_bb, 0);
    if (value_changed)
        MarkItemEdited(id);
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    SameLine(window->Size.x - 30);
    flags |= ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_NoLabel | ImGuiColorEditFlags_NoTooltip | ImGuiColorEditFlags_NoSidePreview | ImGuiColorEditFlags_NoDragDrop | ImGuiColorEditFlags_NoOptions | ImGuiColorEditFlags_AlphaBar | ImGuiColorEditFlags_PickerHueBar;
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    if (alpha_bar)
    {
        ...
        RenderColorRectWithAlphaCheckerboard(bar1_bb.Min, bar1_bb.Max, IM_COL32(0, 0, 0, 0), bar1_bb.GetWidth() / 2.0f, ImVec2(0.0f, 0.0f), 0, 0);
        draw_list->AddRectFilledMultiColor(bar1_bb.Min, bar1_bb.Max, col32_no_alpha, col32_no_alpha, col32_no_alpha & ~IM_COL32_A_MASK, col32_no_alpha & ~IM_COL32_A_MASK);
        float bar1_line_y = (float)(int)(picker_pos.y + (1.0f - alpha) * sv_picker_size + 0.5f);
        RenderFrameBorder(bar1_bb.Min, bar1_bb.Max, 0.0f);
        RenderArrowsForVerticalBar(draw_list, ImVec2(bar1_pos_x - 1, bar1_line_y), ImVec2(bars_triangles_half_sz + 1, bars_triangles_half_sz), bars_width + 2.0f, 0.8f);
    }
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    if (hovered || selected) 
        PushStyleColor(ImGuiCol_Text, ImVec4(ctx.m_settings.menu_color[0], ctx.m_settings.menu_color[1], ctx.m_settings.menu_color[2], 1.f));
...
static ImGuiTabItem* ImGui::TabBarTabListPopupButton(ImGuiTabBar* tab_bar)
{
    ...
    PushStyleColor(ImGuiCol_Text, arrow_col);
    PushStyleColor(ImGuiCol_Button, ImVec4(0, 0, 0, 0));
    bool open = BeginCombo("##v", NULL, ImGuiComboFlags_NoPreview | ImGuiComboFlags_HeightLargest, 3);
    PopStyleColor(2);
