...
#include <sdk/vector/CUtlVector.hpp>
...
struct LayerData_t {
	...
	float m_flFeetCycleRate;
	float ToleranceDef;
	float ToleranceRight;
	float ToleranceLeft;
};
class C_Tickrecord
{
public:
	...
	*/
	...
	float feet_weight_server[3];
	float feet_weight_rebuild[3];
};
...
public:
	...
	virtual void RebuiltLayer6(C_BasePlayer* player, int side, float abs, C_Tickrecord* record);
