...
namespace Engine
{
	...
	void Movement::Begin(CUserCmd* cmd, bool& send_packet)
	{
		...
		{
			if (ctx.m_settings.movement_bunny_hop) {
			...
			else if (ctx.m_settings.movement_air_strafe && !ground && ctx.m_local()->m_MoveType() != MOVETYPE_LADDER)
			{
				auto get_velocity_degree = [](const float length_2d)
				{
					auto tmp = RAD2DEG(atan2(100.f - ctx.m_settings.movement_air_strafe_smoothing, length_2d));
				...
				if ((forwardmove != 0.0f || sidemove != 0.0f) && ctx.m_settings.movement_air_strafe_direction.at(1))
				{
