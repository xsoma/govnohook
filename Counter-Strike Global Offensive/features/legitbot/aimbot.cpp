...
void c_legitaimbot::run(CUserCmd* cmd)
{
	...
	if (!ctx.m_settings.aimbot[weapon_index].betweenShots && active_weapon->m_flNextPrimaryAttack() > TICKS_TO_TIME(ctx.fixed_tickbase_backtrack))
		return;
		...
		&& active_weapon->GetInaccuracy() <= ctx.m_settings.aimbot[weapon_index].maxAimInaccuracy)
	{
		...
		const auto aim_punch = ctx.m_local()->m_aimPunchAngle() * ((csgo.m_engine_cvars()->FindVar(sxor("weapon_recoil_scale"))->GetFloat() / 100) * ctx.m_settings.aimbot[weapon_index].rcsfov);
			...
			for (auto bone : { 8, 4, 3, 7, 6, 5 })
			{
				...
				if (!is_visible(entity, local_player_eye_position, bone_position) 
					&& (ctx.m_settings.aimbot[weapon_index].visibleOnly 
						/*|| penetration::run(local_player_eye_position, bone_position, ctx.m_local(), entity, 0) >= mdmg*/))
					continue;
			...
			&& !best_target.IsZero())
		{
			...
			if (ctx.m_settings.aimbot[weapon_index].autoScope 
				&& active_weapon->m_flNextPrimaryAttack() <= TICKS_TO_TIME(ctx.fixed_tickbase_backtrack)
