	...
	if (damages.size() > 0)
	{
		...
		for (auto idmg : damages)
		{
			...
			if (Drawing::WorldToScreen(prev, screen))
			{
				...
				if (info < 100.f)
					Drawing::DrawString(F::ESPInfo, screen.x, screen.y + 5, info >= idmg.first->m_iHealth() ? Color{ 255, 0, 0, 255 } : Color{ 255, 255, 255, 255 }, FONT_CENTER, sxor("-%i"), (int)idmg.second);
				else
					Drawing::DrawString(F::ESPInfo, screen.x, screen.y + 5, Color(255, 255, 255, 255), FONT_CENTER, sxor("%.1fm"), float(idmg.second - 1337.f));
			}
		}
	}
...
void c_grenade_tracer::think(CUserCmd* cmd) {
	...
	if (!ctx.m_settings.other_esp_grenade_trajectory)
		return;
...
void c_grenade_tracer::simulate(CUserCmd* cmd) {
	...
	m_collision_ent = nullptr;
	m_path.emplace_back(m_start);
	m_bounces.emplace_back(m_start, Color(255, 255, 255, 255));
}
...
void c_grenade_tracer::ResolveFlyCollisionBounce(CGameTrace& trace, size_t* tick, size_t *flags) {
	...
	m_bounces.emplace_back(trace.endpos, Color(255, 255, 255, 255));
}
