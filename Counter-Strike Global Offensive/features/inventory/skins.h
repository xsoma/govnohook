...
struct KnifeData {
	...
	__forceinline void setup(int id, const std::string& model, const std::string& world_model, const std::string& name) {
		...
		m_model = model;
		m_name = name;
		m_model_index = csgo.m_model_info()->GetModelIndex(model.data());
		m_world_model_index = csgo.m_model_info()->GetModelIndex(world_model.data());
	}
};
struct GloveData {
	int         m_id;
...
public:
	void load();
	void think();
	void UpdateItem(C_BaseCombatWeapon* item);
	void UpdateAnimations(C_WeaponCSBaseGun* ent);
	void init();
	void UpdateHud();



	bool ApplyCustomSkin(C_BasePlayer* localPlayer, C_WeaponCSBaseGun* pWeapon, short nWeaponIndex);
