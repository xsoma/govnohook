...
IMaterial* IMaterialSystem::CreateMaterial(const char* name, KeyValues* key)
{
	using Fn = IMaterial * (__thiscall*)(void*, const char*, KeyValues*);
	return Memory::VCall< Fn >(this, 83)(this, name, key);
}
...
void IEngineTrace::TraceLine(const Vector& src, const Vector& dst, int mask, IHandleEntity* entity, int collision_group, CGameTrace* trace) {
	static auto trace_filter_simple = Memory::Scan(sxor("client.dll"), ("55 8B EC 83 E4 F0 83 EC 7C 56 52")) + 0x3D;
