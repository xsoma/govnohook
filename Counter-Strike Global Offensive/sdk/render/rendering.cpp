...
#include <icons/icon_font.hpp>

#include "imgui/imgui_impl_win32.h"
#include "imgui/imgui_impl_dx9.h"
#include <icons/i_menu_texture.h>
...
namespace d
{
	...
	bool create(IDirect3DDevice9* device)
	{
		...
		D3DXCreateTextureFromFileInMemoryEx(device, &texture, sizeof(texture), 1920, 1080, D3DX_DEFAULT, 0, D3DFMT_UNKNOWN, D3DPOOL_DEFAULT, D3DX_DEFAULT, D3DX_DEFAULT, 0, NULL, NULL, &menu::m_pTexture);

		io.Fonts->AddFontFromFileTTF(sxor("C:/windows/fonts/segoeui.ttf"), 16.f, &cfg, io.Fonts->GetGlyphRangesCyrillic());
		io.Fonts->AddFontFromMemoryTTF(keybinds_font, 25600, 12.f, &cfg, io.Fonts->GetGlyphRangesCyrillic());
		io.Fonts->AddFontFromFileTTF(sxor("C:/windows/fonts/segoeui.ttf"), 20.f, &cfg, io.Fonts->GetGlyphRangesCyrillic());
		io.Fonts->AddFontFromFileTTF(sxor("C:/windows/fonts/tahoma.ttf"), 14.f, &cfg, io.Fonts->GetGlyphRangesCyrillic());
		io.Fonts->AddFontFromFileTTF(sxor("C:/windows/fonts/verdanab.ttf"), 18.f, &cfg, io.Fonts->GetGlyphRangesCyrillic());
		io.Fonts->AddFontFromMemoryTTF((void*)configsandscripts, sizeof(configsandscripts), 15.f, &cfg, io.Fonts->GetGlyphRangesCyrillic());
		io.Fonts->AddFontFromFileTTF(sxor("C:/windows/fonts/verdanab.ttf"), 24.f, &cfg, io.Fonts->GetGlyphRangesCyrillic());
		io.Fonts->AddFontFromFileTTF(sxor("c:/windows/fonts/verdana.ttf"), 13.f, &cfg, io.Fonts->GetGlyphRangesCyrillic());
		io.Fonts->AddFontFromMemoryTTF(weapons_font, 47320, 14.f, &cfg, io.Fonts->GetGlyphRangesCyrillic());

		io.Fonts->AddFontFromMemoryTTF((void*)MuseoSansCyrl300, sizeof(MuseoSansCyrl300), 15.f, &cfg, io.Fonts->GetGlyphRangesCyrillic());
		io.Fonts->AddFontFromMemoryTTF((void*)MuseoSansCyrl500, sizeof(MuseoSansCyrl500), 20.f, &cfg, io.Fonts->GetGlyphRangesCyrillic());
		io.Fonts->AddFontFromMemoryTTF((void*)MuseoSansCyrl700, sizeof(MuseoSansCyrl700), 30.f, &cfg, io.Fonts->GetGlyphRangesCyrillic());
	...
	bool render()
	{
		...
		ImGui_ImplDX9_RenderDrawData(&m_drawdata);
		ImGui::Render();
		ImGui::EndFrame();
